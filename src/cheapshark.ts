...
// ================== INTERFACES ==================
export interface JuegoCheapShark {
  gameID: string;
  steamAppID: string;
  cheapest: string;
 ...
 */
export async function buscarJuegoPorNombre(titulo: string): Promise<JuegoCheapShark[]> {
  try {
    ...
    const response = await fetch(
      `${CHEAPSHARK_URL}/games?title=${encodeURIComponent(titulo)}&limit=15`
    );
 ...
 */
export async function obtenerDetallesJuego(gameID: string): Promise<JuegoDetalle | null> {
  try {
 ...
 */
export async function obtenerMejoresOfertas(limite: number = 20): Promise<any[]> {
  try {
    const response = await fetch(
      `${CHEAPSHARK_URL}/deals?sortBy=Savings&pageSize=${limite}`
    );
 ...
 */
export const TIENDAS_CHEAPSHARK: { [key: string]: string } = {
  "1": "Steam",
  "2": "GamersGate",
  "3": "GreenManGaming",
  "7": "GOG",
